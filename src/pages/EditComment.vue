<template>
  <q-page class="flex column justify-start items-center">
    ...
    <div class="comment-fields">
      ...
      <q-btn class="add-comment-btn" label="Editar Comentário" @click="addComment"
             no-caps dark/>
    </div>
  </q-page>
</template>
...
export default {
  ...
  methods: {
    async addComment() {
      ...
      if (resp) {
        notify('Comentário editado com sucesso', TypeMessage.success);
        this.$router.back();
      }
    },
  },
  async mounted() {
    this.comment = await this.service.find(this.$route.params.id);

    if (this.comment.reply_comment) {
      this.selectedComment = this.comment.reply_comment;
    }
